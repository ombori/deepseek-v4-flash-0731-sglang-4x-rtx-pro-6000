...
## 2026-08-08 update: scheduler livelock fix, tool-call parser hardening, opt-in logprobs

Three fixes across five diffs (rows at the bottom of the [patch table](#patches)), none touching the decode or prefill hot path — no change expected to any number in [Measured performance](#measured-performance) or [Concurrency and operating points](#concurrency-and-operating-points).

- **A second, distinct SWA-adjacent scheduler livelock, fixed.** Not the KV-eviction bug from the 2026-08-07 update (#33805, already shipped) — this one is in admission. `_swa_budget_for_req` charged a full sliding window's worth of decode headroom to a resume whose window was already funded by its locked radix-cache prefix. Under `--schedule-policy lpm`, a pool sized to ~2 windows rejects that candidate forever — same request re-checked, same rejection, spinning the scheduler at 100% CPU while the GPU sits idle. It needs hybrid SWA + LPM + a radix-cached prefix resume to trigger, which is exactly this recipe's shape, and the prefix-affinity dispatch from the 2026-08-06 update makes it *more* likely to trigger, not less — pinning conversations to their prefix-owning rank turns cached-prefix resumes from occasional into routine. Fixed by backporting merged upstream PRs [#32379](https://github.com/sgl-project/sglang/pull/32379) and [#32700](https://github.com/sgl-project/sglang/pull/32700) (the latter restricts a related SWA chunk-cap escape hatch to genuine head-of-line livelock, so it stops firing on merely transient pressure and forcing running requests to retract). Scheduler-only; the decode and prefill kernels are untouched.
- **A second escape closed in the streaming tool-call parser.** The 2026-07-31 incident (buffer poisoning via a `MalformedJSON` escape, `pr32332-notest.diff` above) had a sibling: `partial_json_parser.fix_fast()` can raise a bare `AssertionError` on some partial inputs instead of the exception type the handler catches, which sails past it into the same catch-all and reopens the identical failure — raw DSML text returned as normal content, buffer never cleared, re-emitted and growing on every subsequent token. Different code path, same failure shape. Fixed via merged upstream [#31975](https://github.com/sgl-project/sglang/pull/31975).
- **`return_logprob` now works under DSPARK.** Stock v0.5.16 rejects it outright ("DSpark speculative decoding does not support return_logprob yet"). Upstream [#33459](https://github.com/sgl-project/sglang/pull/33459) added this for DFlash three days before we needed it, and deliberately left DSpark rejected — there's no server-level DSpark test upstream to validate it against, not an architectural obstacle. We backported #33459 and wrote the ~15-line DSpark extension it was missing (chain-draft accept indices are a plain `arange`, so no kernel changes were needed). Reports the target model's temperature-scaled distribution at each position, pre-top_k/top_p truncation — useful if you're diagnosing sampling behavior and want to see what the model actually assigned probability to, not just what survived truncation. Entirely opt-in: gated behind the request's own `return_logprob` flag, and every line of the patch sits inside that check, so a deployment that never sets the flag runs identically to before, including cost — we verified this by reading the diff, not just testing the happy path.
...
| 8 | **Scheduler livelock on cached-prefix resumes under LPM — fixed in this image.** Distinct from #33805 above (that one is eviction; this one is admission). `_swa_budget_for_req` charged a resume the full sliding-window cost even though its window was already funded by its locked radix-cache prefix; at a ~2-window pool the recheck rejects it forever, spinning the scheduler at 100% CPU while the GPU idles. Needs hybrid SWA + `--schedule-policy lpm` + a cached-prefix resume — this recipe's exact shape, and prefix-affinity dispatch (2026-08-06) makes it more likely to trigger by pinning conversations onto their prefix-owning rank. | fixed by backporting merged [#32379](https://github.com/sgl-project/sglang/pull/32379) + [#32700](https://github.com/sgl-project/sglang/pull/32700) |
...
| `pr33795-notest.diff` | [#33795](https://github.com/sgl-project/sglang/pull/33795) MERGED | missing `synchronize()` before the TP barrier in the CUDA-graph runner backends (full + piecewise) on a rank-divergent path. Already running in production; this repo's patch set had gone stale by one file — added here for parity, no serving-config change |
...
| `pr33459-notest.diff` | [#33459](https://github.com/sgl-project/sglang/pull/33459) MERGED 2026-08-05 | adds `return_logprob` support for DFlash spec decode — reports the target model's temperature-scaled distribution per position (pre-truncation), via the existing EAGLE/NGRAM `compute_spec_v2_logprobs` machinery. Deliberately left DSpark rejected upstream (no server-level DSpark test to validate it against, not an architectural obstacle) |
| `dspark-return-logprob.diff` | ours — extends #33459 to DSpark; upstream-PR candidate | removes DSpark's `return_logprob` rejection and wires the same `compute_spec_v2_logprobs` call after `accept_and_finalize`. Chain-draft accept indices are a plain `arange` over the strided verify rows, so no kernel changes are needed. Gated entirely behind the request's `return_logprob` flag; verified every added line sits inside that check, so cost is zero unless a client asks for it |
...
| `pr32700-notest.diff` | [#32700](https://github.com/sgl-project/sglang/pull/32700) MERGED 2026-08-06 | restricts the SWA chunk-cap escape hatch to genuine head-of-line livelock (adds a "never fits at all" check); today it also fires on merely transient pressure, which admits into decode headroom and forces running requests to retract. Companion to `pr32379-notest.diff` below, applied after it |
...
| `pr32379-notest.diff` | [#32379](https://github.com/sgl-project/sglang/pull/32379) MERGED 2026-07-25 | SWA admission livelock on cached-prefix resumes: `_swa_budget_for_req` charges a full sliding window to a resume already funded by its locked radix-cache prefix, so a ~2-window pool rejects it forever under `--schedule-policy lpm` — scheduler spins at 100% CPU, GPU idle. Needs hybrid SWA + LPM + a cached-prefix resume (this recipe's shape); prefix-affinity dispatch makes resumes hit their prefix-owning rank far more often, raising exposure. Fixes known issue 8 |
...
| `pr31975-notest.diff` | [#31975](https://github.com/sgl-project/sglang/pull/31975) MERGED 2026-07-22 | closes a second escape into the same buffer-poisoning loop as `pr32332-notest.diff` above: `partial_json_parser.fix_fast()` can raise a bare `AssertionError` on some partial inputs, which sails past both handlers into the streaming catch-all. Different file, composes cleanly with the existing fix |
...
Everything in this table, including the nine 2026-08-07 rows and the five 2026-08-08 rows, is in the published image. Retire each backport when it lands in an sglang release. The stack also rebases onto upstream `main` (the merged diffs drop, the flashinfer pin becomes upstream's own), but note that `main` still lacks #29927's hc_combine, so known issue 1 is live there.
